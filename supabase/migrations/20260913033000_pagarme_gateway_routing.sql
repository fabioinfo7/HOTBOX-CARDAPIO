-- HotBox Delivery — Pagar.me + roteamento independente Pix/Cartão
begin;

-- Gateways por forma de pagamento. Os defaults herdam o provedor antigo.
alter table public.store_config add column if not exists digital_pix_provider text;
alter table public.store_config add column if not exists digital_card_provider text;
...
alter table public.store_config add constraint store_config_digital_payment_provider_ck
  check (digital_payment_provider in ('infinitepay','mercadopago','pagarme','appmax'));
update public.store_config
set digital_pix_provider = coalesce(nullif(digital_pix_provider,''), coalesce(digital_payment_provider,'infinitepay')),
    digital_card_provider = coalesce(nullif(digital_card_provider,''), coalesce(digital_payment_provider,'infinitepay'))
where id=1;
alter table public.store_config alter column digital_pix_provider set default 'infinitepay';
alter table public.store_config alter column digital_card_provider set default 'infinitepay';
...
alter table public.store_config add constraint store_config_digital_pix_provider_ck
  check (digital_pix_provider is null or digital_pix_provider in ('infinitepay','mercadopago','pagarme','appmax'));
...
alter table public.store_config add constraint store_config_digital_card_provider_ck
  check (digital_card_provider is null or digital_card_provider in ('infinitepay','mercadopago','pagarme','appmax'));

-- Pagar.me V5. A secret key nunca é exposta pela RPC pública.
alter table public.store_config add column if not exists pagarme_enabled boolean not null default false;
alter table public.store_config add column if not exists pagarme_public_key text;
alter table public.store_config add column if not exists pagarme_secret_key text;
alter table public.store_config add column if not exists pagarme_webhook_token text;
alter table public.store_config add column if not exists pagarme_max_installments smallint not null default 1;
update public.store_config set pagarme_webhook_token=coalesce(nullif(pagarme_webhook_token,''),gen_random_uuid()::text) where id=1;
alter table public.store_config drop constraint if exists store_config_pagarme_max_installments_ck;
alter table public.store_config add constraint store_config_pagarme_max_installments_ck check (pagarme_max_installments between 1 and 12);

-- Auditoria do Pagar.me no checkout.
alter table public.site_checkout_sessions add column if not exists pagarme_order_id text;
alter table public.site_checkout_sessions add column if not exists pagarme_status text;
alter table public.site_checkout_sessions add column if not exists pagarme_charge_id text;
alter table public.site_checkout_sessions add column if not exists pagarme_transaction_id text;
alter table public.site_checkout_sessions add column if not exists pagarme_qr_code text;
alter table public.site_checkout_sessions add column if not exists pagarme_qr_code_url text;
alter table public.site_checkout_sessions add column if not exists pagarme_attempt_no integer not null default 0;
alter table public.site_checkout_sessions add column if not exists pagarme_verified_at timestamptz;
alter table public.site_checkout_sessions add column if not exists pagarme_webhook_payload jsonb;
alter table public.site_checkout_sessions add column if not exists pagarme_verification_payload jsonb;
create unique index if not exists idx_site_checkout_pagarme_order on public.site_checkout_sessions(pagarme_order_id) where pagarme_order_id is not null;
...
alter table public.site_checkout_sessions add constraint site_checkout_payment_provider_ck
  check (payment_provider is null or payment_provider in ('infinitepay','mercadopago','pagarme','appmax','stripe','pay_on_delivery'));
...
-- Configuração pública segura: devolve somente chaves públicas e roteamento.
...
begin
  ...
  v_method := case
    when c.payment_kind in ('stripe_pix','infinitepay_pix','mercadopago_pix','pagarme_pix') then 'pix'
    else 'card'
  end;
...
-- -----------------------------------------------------------------------------
-- FINANCEIRO DO CARDÁPIO — TODOS OS GATEWAYS DIGITAIS
-- -----------------------------------------------------------------------------
...
as $$
  with base as (
    ...
    where status='paid'
      ...
      and (
        ...
        or (p_payment_kind='pix' and payment_kind in ('infinitepay_pix','mercadopago_pix','pagarme_pix','appmax_pix'))
        or (p_payment_kind='card' and payment_kind in ('infinitepay','infinitepay_card','mercadopago','mercadopago_card','pagarme','pagarme_card','appmax','appmax_card'))
      )
  ), filtered as (
    select * from base
  ...
  select jsonb_build_object(
    ...
    'pix_total',coalesce(sum(case when payment_kind in ('infinitepay_pix','mercadopago_pix','pagarme_pix','appmax_pix') then amount else 0 end),0),
    'pix_count',count(*) filter(where payment_kind in ('infinitepay_pix','mercadopago_pix','pagarme_pix','appmax_pix')),
    'card_total',coalesce(sum(case when payment_kind in ('infinitepay','infinitepay_card','mercadopago','mercadopago_card','pagarme','pagarme_card','appmax','appmax_card') then amount else 0 end),0),
    'card_count',count(*) filter(where payment_kind in ('infinitepay','infinitepay_card','mercadopago','mercadopago_card','pagarme','pagarme_card','appmax','appmax_card')),
