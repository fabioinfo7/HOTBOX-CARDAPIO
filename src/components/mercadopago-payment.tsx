...
export function MercadoPagoPayment({ checkoutId, amount, publicKey, maxInstallments = 1, customerEmail, environment = "production", origin, supportWhatsappUrl, allowedMethod = "both", onSwitchToPix, onPaid, onCancel }: Props) {
  ...
  const rejectionTrackedRef = useRef<Set<string>>(new Set());

  function friendlyRejectionReason(code: unknown, fallback?: unknown) {
    const detail = String(code || "").trim();
    const map: Record<string, string> = {
      card_not_authorized: "Cartão não autorizado pelo Mercado Pago ou pelo banco",
      cc_rejected_bad_filled_card_number: "Número do cartão não foi aceito",
      cc_rejected_bad_filled_date: "Validade do cartão não foi aceita",
      cc_rejected_bad_filled_security_code: "Código de segurança não foi aceito",
      cc_rejected_insufficient_amount: "Cartão sem limite disponível",
      cc_rejected_call_for_authorize: "Banco pediu autorização para a compra",
      cc_rejected_card_disabled: "Cartão bloqueado ou desativado",
      cc_rejected_duplicated_payment: "Tentativa considerada duplicada",
      cc_rejected_high_risk: "Compra recusada pela análise de segurança",
      cc_rejected_3ds_challenge: "Confirmação de segurança do banco não foi concluída",
    };
    return map[detail] || String(fallback || "Pagamento no cartão não foi aprovado");
  }

  function trackRejectedPayment(input: {
    key: string;
    ...
    if (!input.key || rejectionTrackedRef.current.has(input.key)) return;
    rejectionTrackedRef.current.add(input.key);
    ...
    trackAnalytics("payment_failed", {
      ...
      properties: {
        ...
        payment_type: "card",
        reason_code: String(input.statusDetail || "card_not_authorized"),
        reason_friendly: friendlyRejectionReason(input.statusDetail, input.message),
        payment_id: input.paymentId ? String(input.paymentId) : null,
        payment_method_id: input.paymentMethodId ? String(input.paymentMethodId) : null,
        attempt_id: input.attemptId ? String(input.attemptId) : null,
      },
    });
  }
    ...
    async function init() {
      try {
        ...
        controller = await bricks.create("payment", brickContainerId, {
          ...
          customization: {
            paymentMethods: {
              ...(allowedMethod !== "card" ? { bankTransfer: "all" } : {}),
              ...(allowedMethod !== "pix" ? { creditCard: "all", minInstallments: 1, maxInstallments: Math.min(12, Math.max(1, Number(maxInstallments || 1))) } : {}),
            },
          },
          callbacks: {
            ...
            onSubmit: async ({ formData }: any) => {
              ...
              if (!result?.ok) {
                if (result?.rejected) {
                  setLastRejected(result?.error || "O cartão não foi aprovado.");
                  const methodId = String(
                    formData?.payment_method_id ||
                      formData?.paymentMethodId ||
                      "",
                  );
                  if (methodId && methodId !== "pix") {
                    trackRejectedPayment({
                      key: `submit:${attemptId}`,
  ...
  }, [checkoutId, amount, publicKey, maxInstallments, customerEmail, environment, origin, allowedMethod, brickKey, brickContainerId]);
    ...
    const timer = window.setInterval(async () => {
      ...
      try {
        ...
        if (result.rejected) {
          ...
          window.clearInterval(timer);
          trackRejectedPayment({
            key: `poll:${pending.paymentId}:${String(result.statusDetail || "rejected")}`,
            statusDetail: result.statusDetail,
          ...
          setPending(null);
          setReady(false);
          setLastRejected(result.message || "O pagamento não foi aprovado.");
          setBrickKey((k) => k + 1);
          toast.error(result.message || "O pagamento não foi aprovado. Escolha outra opção para continuar.");
          return;
        }
  ...
  async function manualCheck() {
    ...
    try {
      ...
      if (result?.rejected) {
        trackRejectedPayment({
          key: `manual:${pending.paymentId}:${String(result.statusDetail || "rejected")}`,
          statusDetail: result.statusDetail,
          message: result.message,
          paymentId: pending.paymentId,
        });
        setPending(null);
        setReady(false);
        setLastRejected(result.message || "Pagamento não aprovado.");
        setBrickKey((k) => k + 1);
        toast.error(result.message || "Pagamento não aprovado. Escolha outra opção para continuar.");
  ...
  if (lastRejected && allowedMethod === "card") {
    return (
      <div className="space-y-4 rounded-3xl border border-red-200 bg-red-50 p-5">
        <div className="flex items-start gap-3">
          ...
          <div><h3 className="font-black text-red-950">Cartão não aprovado</h3><p className="mt-1 text-sm text-red-900">{lastRejected}</p></div>
        </div>
        <div className="grid gap-2 sm:grid-cols-3">
          <Button className="rounded-xl" onClick={() => { setLastRejected(""); setReady(false); setBrickKey((k) => k + 1); }}><RefreshCw className="mr-2 size-4" /> Outro cartão</Button>
          {onSwitchToPix && <Button variant="outline" className="rounded-xl border-emerald-300 bg-white text-emerald-800" onClick={onSwitchToPix}><QrCode className="mr-2 size-4" /> Pagar com Pix</Button>}
          {supportWhatsappUrl && <a href={supportWhatsappUrl} target="_blank" rel="noreferrer"><Button variant="outline" className="w-full rounded-xl border-[#25D366]/40 bg-white text-[#128C4A]"><MessageCircle className="mr-2 size-4" /> Link pelo WhatsApp</Button></a>}
        </div>
      </div>
    );
  }
