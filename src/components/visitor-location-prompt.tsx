...
export function VisitorLocationPrompt() {
  ...
  const dismiss = (eventName: string) => {
    try { sessionStorage.setItem(PROMPT_KEY, "1"); } catch { /* ignora */ }
    setVisible(false);
    trackAnalytics(eventName, { event_category: "location" });
  };

  const allow = () => {
    if (!navigator.geolocation) {
      dismiss("visitor_location_unavailable");
      return;
    }
    ...
    <div className="fixed inset-x-3 bottom-3 z-[80] mx-auto max-w-md rounded-2xl border bg-background p-4 shadow-xl sm:bottom-5">
      <div className="flex gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Com sua permissão, usamos a localização aproximada apenas para entender os bairros que visitam a loja. Isso não altera taxa nem disponibilidade de entrega.</p>
