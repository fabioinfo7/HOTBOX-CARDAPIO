...
export const fetchOverpassCandidatesFn = createServerFn({ method: "POST" }).handler(async () => {
  ...
  const { loadTiers } = await import("./delivery-distance.server");
  ...
  const tiers = await loadTiers(supabaseAdmin, { delivery_fee_tiers: cfg.delivery_fee_tiers ?? [] } as any);
...
export const resolveZonaBatchFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { getRoadDistanceKm, loadTiers, pickTier } = await import("./delivery-distance.server");
