...
export const sendChatText = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // Se o operador escolheu "Eu vou informar" no popup de frete, a primeira
    // mensagem manual com valor vira a fonte oficial. Se a automação já enviou
    // a taxa, bloqueia um segundo valor no mesmo endereço.
    const { captureOperatorFreightMessage } = await import("./freight-approval.server");
    const freightGuard = await captureOperatorFreightMessage(supabaseAdmin, {
      conversationId: data.conversationId,
      text: data.text,
      commit: false,
    });
    if (freightGuard.blocked) return { error: freightGuard.error };
    ...
    if (freightGuard.captured) {
      await captureOperatorFreightMessage(supabaseAdmin, {
        conversationId: data.conversationId,
        text: data.text,
        commit: true,
      });
    ...
    const { resolvePendingHumanHandoffsForConversation } = await import("./human-handoff.server");
    await resolvePendingHumanHandoffsForConversation(supabaseAdmin, data.conversationId, "assumed");
    await touchConversation(supabaseAdmin, data.conversationId, data.text);

    return { ok: true, freightCaptured: !!freightGuard.captured };
  });
