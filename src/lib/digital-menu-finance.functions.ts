...
const isProviderFilter = (value: unknown): value is "all" | "infinitepay" | "mercadopago" | "pagarme" | "efi" | "appmax" =>
  value === "all" || value === "infinitepay" || value === "mercadopago" || value === "pagarme" || value === "efi" || value === "appmax";

const DIGITAL_KINDS = ["infinitepay", "infinitepay_card", "infinitepay_pix", "mercadopago", "mercadopago_card", "mercadopago_pix", "pagarme", "pagarme_card", "pagarme_pix", "efi", "efi_card", "efi_pix", "appmax", "appmax_card", "appmax_pix"];

function applyPaymentFilter(query: any, payment: "all" | "pix" | "card") {
  if (payment === "pix") return query.in("payment_kind", ["infinitepay_pix", "mercadopago_pix", "pagarme_pix", "efi_pix", "appmax_pix"]);
  if (payment === "card") return query.in("payment_kind", ["infinitepay", "infinitepay_card", "mercadopago", "mercadopago_card", "pagarme", "pagarme_card", "efi", "efi_card", "appmax", "appmax_card"]);
  return query.in("payment_kind", DIGITAL_KINDS);
}

function applyProviderFilter(query: any, provider: "all" | "infinitepay" | "mercadopago" | "pagarme" | "efi" | "appmax") {
...
export const listDigitalMenuFinanceFn = createServerFn({ method: "POST" })
  ...
  .inputValidator((data: {
    ...
    provider?: "all" | "infinitepay" | "mercadopago" | "pagarme" | "efi" | "appmax";
    page?: number;
    pageSize?: number;
  }) => data)
  .handler(async ({ data, context }) => {
    ...
    let q = (supabaseAdmin as any)
      ...
      .select(
        "id,status,payment_provider,payment_kind,customer_name,customer_phone,subtotal,delivery_fee,coupon_code,coupon_discount,total,order_id,paid_at,created_at,updated_at,infinitepay_receipt_url,infinitepay_amount_cents,infinitepay_paid_amount_cents,infinitepay_installments,mercadopago_status,mercadopago_status_detail,mercadopago_payment_method_id,mercadopago_payment_type_id,mercadopago_installments,mercadopago_transaction_amount,mercadopago_net_received_amount,mercadopago_fee_amount,pagarme_order_id,pagarme_status,pagarme_charge_id,pagarme_transaction_id,efi_charge_id,efi_txid,efi_status,finance_reference,finance_note",
        { count: "exact" },
      )
      .eq("status", "paid")
