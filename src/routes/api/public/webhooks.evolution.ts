  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // Esta integração usa a API clássica de Payments (/v1/payments).
        // O simulador do Mercado Pago pode enviar eventos da nova Orders API
        // (type=order / action=order.*). Nesse caso, data.id é o ID da Order,
        // não um payment_id válido para /v1/payments. Apenas confirmamos o
        // recebimento para que o Mercado Pago não trate isso como falha.
        ...
        const result = await finalizeIfApproved(supabaseAdmin, checkout, payment, async (orderId: string) => {
