...
function ObrigadoPage() {
  function trackPurchaseOnce(order: string | null, checkout: string | null, total: number | null, method: string) {
    ...
    trackAnalytics("purchase", { event_category: "commerce", order_id: order, checkout_id: checkout, value: Number(total || 0), payment_method: method });
  }
  ...
  useEffect(() => {
    ...
    async function confirmPagarme() {
      const directOrderId = params.get("order_id") || "";
      const checkoutId = params.get("checkout_id") || "";
      if (directOrderId) {
        setOrderId(directOrderId);
        trackPurchaseOnce(directOrderId, checkoutId || null, null, "pagarme");
        if (alive) setState("paid");
        return;
      }
      if (!checkoutId) { if (alive) setState("pending"); return; }
      try {
        const { checkPagarmePayment } = await import("@/lib/pagarme.functions");
        const checked: any = await checkPagarmePayment({ data: { checkoutId } });
        if (!alive) return;
        if (checked?.approved && checked?.order_id) {
          setOrderId(String(checked.order_id));
          trackPurchaseOnce(String(checked.order_id), checkoutId, null, "pagarme");
          setState("paid");
        } else setState("pending");
      } catch { if (alive) setState("pending"); }
    }

    async function confirmEfi() {
      const directOrderId = params.get("order_id") || "";
      const checkoutId = params.get("checkout_id") || "";
      if (directOrderId) {
        setOrderId(directOrderId);
        trackPurchaseOnce(directOrderId, checkoutId || null, null, "efi");
        if (alive) setState("paid");
        return;
      }
      if (!checkoutId) { if (alive) setState("pending"); return; }
      try {
        const { checkEfiPayment } = await import("@/lib/efi.functions");
        const checked: any = await checkEfiPayment({ data: { checkoutId } });
        if (!alive) return;
        if (checked?.approved && checked?.order_id) {
          setOrderId(String(checked.order_id));
          trackPurchaseOnce(String(checked.order_id), checkoutId, null, "efi");
          setState("paid");
        } else setState("pending");
      } catch { if (alive) setState("pending"); }
    }
