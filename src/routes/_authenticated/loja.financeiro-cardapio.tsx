...
type ProviderFilter = "all" | "mercadopago" | "infinitepay" | "pagarme" | "efi" | "appmax";
...
type Tx = {
  ...
  pagarme_order_id?: string | null; pagarme_status?: string | null; pagarme_charge_id?: string | null; pagarme_transaction_id?: string | null; efi_charge_id?: string | null; efi_txid?: string | null; efi_status?: string | null;
...
function providerOf(tx: Tx): "mercadopago" | "infinitepay" | "pagarme" | "efi" | "appmax" {
  if (tx.payment_provider === "pagarme" || tx.payment_kind.startsWith("pagarme")) return "pagarme";
  if (tx.payment_provider === "efi" || tx.payment_kind.startsWith("efi")) return "efi";
  if (tx.payment_provider === "appmax" || tx.payment_kind.startsWith("appmax")) return "appmax";
  if (tx.payment_provider === "mercadopago" || tx.payment_kind.startsWith("mercadopago")) return "mercadopago";
  return "infinitepay";
}
function providerLabel(tx: Tx) { const p = providerOf(tx); return p === "pagarme" ? "Pagar.me" : p === "efi" ? "Efí" : p === "appmax" ? "Appmax" : p === "mercadopago" ? "Mercado Pago" : "InfinitePay"; }
function paymentLabel(tx: Tx) { return tx.payment_kind.endsWith("_pix") || tx.mercadopago_payment_method_id === "pix" ? "Pix" : "Cartão"; }
function installments(tx: Tx) { const p = providerOf(tx); return p === "mercadopago" ? Number(tx.mercadopago_installments || 1) : p === "infinitepay" ? Number(tx.infinitepay_installments || 1) : 1; }
function grossAmount(tx: Tx) { const p = providerOf(tx); return p === "mercadopago" ? Number(tx.mercadopago_transaction_amount ?? tx.total) : p === "infinitepay" ? Number(tx.infinitepay_amount_cents == null ? tx.total : tx.infinitepay_amount_cents / 100) : Number(tx.total); }
function customerPaid(tx: Tx) { const p = providerOf(tx); return p === "infinitepay" ? Number(tx.infinitepay_paid_amount_cents == null ? tx.total : tx.infinitepay_paid_amount_cents / 100) : grossAmount(tx); }
