...
type ProviderFilter = "all" | "mercadopago" | "appmax" | "infinitepay";
...
type Tx = {
  ...
  appmax_order_id?: number | null; appmax_status?: string | null; appmax_payment_method?: string | null; appmax_installments?: number | null; appmax_total_paid_cents?: number | null;
...
function providerOf(tx: Tx): "mercadopago" | "appmax" | "infinitepay" {
...
function providerLabel(tx: Tx) { return providerOf(tx) === "mercadopago" ? "Mercado Pago" : providerOf(tx) === "appmax" ? "Appmax" : "InfinitePay"; }
function paymentLabel(tx: Tx) { return tx.payment_kind.endsWith("_pix") || tx.mercadopago_payment_method_id === "pix" || tx.appmax_payment_method === "pix" ? "Pix" : "Cartão"; }
function installments(tx: Tx) { return providerOf(tx) === "mercadopago" ? Number(tx.mercadopago_installments || 1) : providerOf(tx) === "appmax" ? Number(tx.appmax_installments || 1) : Number(tx.infinitepay_installments || 1); }
function grossAmount(tx: Tx) { return providerOf(tx) === "mercadopago" ? Number(tx.mercadopago_transaction_amount ?? tx.total) : providerOf(tx) === "appmax" ? Number(tx.appmax_total_paid_cents == null ? tx.total : tx.appmax_total_paid_cents / 100) : Number(tx.infinitepay_amount_cents == null ? tx.total : tx.infinitepay_amount_cents / 100); }
function customerPaid(tx: Tx) { return providerOf(tx) === "mercadopago" || providerOf(tx) === "appmax" ? grossAmount(tx) : Number(tx.infinitepay_paid_amount_cents == null ? tx.total : tx.infinitepay_paid_amount_cents / 100); }
