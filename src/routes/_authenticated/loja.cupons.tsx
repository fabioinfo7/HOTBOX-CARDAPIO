...
function Metric({ icon: Icon, label, value, sub }: any) {
  return <Card className="hotbox-admin-kpi p-4"><div className="flex items-start gap-3"><div className="rounded-xl bg-muted p-2"><Icon className="size-4" /></div><div><p className="text-xs text-muted-foreground">{label}</p><p className="text-xl font-extrabold">{value}</p>{sub ? <p className="text-[11px] text-muted-foreground">{sub}</p> : null}</div></div></Card>;
}
...
function CouponsPage() {
  ...
  return <div className="hotbox-admin-page space-y-5">
