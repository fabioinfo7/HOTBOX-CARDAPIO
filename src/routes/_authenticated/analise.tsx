...
function AnalisePro() {
  ...
  const tracker = useMemo(() => {
    ...
    const endpoint = `${window.location.origin}/api/public/analise/collect`;
    return `<script>\n(function(w,d){\n  var S=${JSON.stringify(selectedSite.site_key)}, E=${JSON.stringify(endpoint)};\n  var K="analise_pro_"+S, V=localStorage.getItem(K+"_v")||("v_"+crypto.randomUUID()), Q=sessionStorage.getItem(K+"_s")||("s_"+crypto.randomUUID());\n  localStorage.setItem(K+"_v",V); sessionStorage.setItem(K+"_s",Q);\n  var started=Date.now(), maxScroll=0, sent={};\n  function qs(){var q=new URLSearchParams(location.search);return {source:q.get("utm_source")||"direct",medium:q.get("utm_medium")||"none",campaign:q.get("utm_campaign")||null,term:q.get("utm_term")||null,content:q.get("utm_content")||null,click_id:q.get("fbclid")||q.get("gclid")||q.get("ttclid")||null};}\n  function device(){var n=navigator,u=n.userAgent||"";return {device_type:/Mobi|Android|iPhone|iPad/i.test(u)?"mobile":"desktop",browser:/Edg\\//i.test(u)?"Edge":/Chrome\\//i.test(u)?"Chrome":/Firefox\\//i.test(u)?"Firefox":/Safari\\//i.test(u)?"Safari":"Other",os:/Android/i.test(u)?"Android":/iPhone|iPad/i.test(u)?"iOS":/Windows/i.test(u)?"Windows":/Mac OS/i.test(u)?"macOS":"Other",language:n.language,timezone:Intl.DateTimeFormat().resolvedOptions().timeZone,screen_width:screen.width,screen_height:screen.height,viewport_width:innerWidth,viewport_height:innerHeight,network_type:n.connection&&n.connection.effectiveType||null};}\n  function send(name,data){data=data||{};var payload={site_key:S,session_id:Q,visitor_id:V,event_name:name,page_url:location.href,page_path:location.pathname,page_title:d.title,event_data:data,utm:qs(),device:device()};try{navigator.sendBeacon&&navigator.sendBeacon(E,new Blob([JSON.stringify(payload)],{type:"application/json"}));}catch(_){fetch(E,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(payload),keepalive:true}).catch(function(){});}}\n  function once(k,n,data){if(sent[k])return;sent[k]=1;send(n,data);}\n  send("page_view",{referrer:d.referrer||null});\n  addEventListener("scroll",function(){var max=Math.max(1,d.documentElement.scrollHeight-innerHeight);var p=Math.min(100,Math.round(scrollY/max*100));if(p>maxScroll)maxScroll=p;[25,50,75,90,100].forEach(function(m){if(p>=m)once("scroll_"+m,"scroll_depth",{scroll_percent:m});});},{passive:true});\n  addEventListener("click",function(e){var el=e.target.closest&&e.target.closest("a,button,[role='button'],input[type='submit']");if(!el)return;var label=(el.innerText||el.getAttribute("aria-label")||el.getAttribute("data-analytics")||"").trim().slice(0,180);send("click",{label:label,element:el.tagName.toLowerCase(),id:el.id||null,cta:el.getAttribute("data-analytics")||null});if(el.tagName==='A'&&el.href&&new URL(el.href,location.href).origin!==location.origin)send("outbound_click",{href:el.href,label:label});},true);\n  addEventListener("input",function(e){var el=e.target;if(el&&el.matches&&el.matches("input,textarea,select"))once("form_start_"+(el.name||el.id||"field"),"form_start",{field:el.name||el.id||"field"});},true);\n  var tick=setInterval(function(){if(document.visibilityState==='visible'){var sec=Math.round((Date.now()-started)/1000);send("heartbeat",{engaged_seconds:sec,scroll_percent:maxScroll});}},15000);\n  addEventListener("pagehide",function(){clearInterval(tick);send("page_exit",{engaged_seconds:Math.round((Date.now()-started)/1000),scroll_percent:maxScroll});});\n  w.AnalisePro={track:function(name,data){send(name,data||{});},identify:function(data){send("lead",data||{});}};\n})(window,document);\n<\/script>`;
  }, [selectedSite]);
...
function TrackerPanel({ tracker, copied, onCopy }: any) { return <div className="space-y-4"><Panel title="Tracker da página" icon={Code2}><div className="flex flex-wrap items-center gap-2 text-xs text-slate-500"><Check className="size-4 text-emerald-400"/> Pageview <Check className="size-4 text-emerald-400"/> Scroll 25/50/75/90/100 <Check className="size-4 text-emerald-400"/> Cliques <Check className="size-4 text-emerald-400"/> Tempo engajado <Check className="size-4 text-emerald-400"/> UTM/fbclid/gclid</div><div className="relative mt-4"><pre className="max-h-[480px] overflow-auto rounded-2xl border border-white/10 bg-black/30 p-4 text-[11px] leading-5 text-slate-400">{tracker || "Crie uma página para gerar o tracker."}</pre><button onClick={onCopy} disabled={!tracker} className="absolute right-3 top-3 inline-flex items-center gap-2 rounded-lg bg-cyan-400 px-3 py-2 text-xs font-black text-slate-950">{copied?<Check className="size-4"/>:<Clipboard className="size-4"/>}{copied?"Copiado":"Copiar tracker"}</button></div><p className="mt-3 text-xs leading-5 text-slate-500">Cole o código imediatamente antes de &lt;/head&gt; da página de vendas. Para registrar compra/conversão, use <code className="text-cyan-300">AnalisePro.track("purchase", &#123;value: 97&#125;)</code> após a confirmação.</p></Panel></div>; }
