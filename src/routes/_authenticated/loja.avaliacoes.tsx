...
function SatisfactionDashboard() {
  ...
  return (
    <div className="hotbox-admin-page space-y-5 p-1">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Card className="hotbox-admin-kpi p-4"><p className="text-xs text-muted-foreground">Avaliação geral</p><div className="mt-1 flex items-end gap-2"><p className="text-3xl font-extrabold">{overallPercent}%</p><span className="mb-1 text-sm text-muted-foreground">{overall.toFixed(2)}/5</span></div><Stars value={overall} /></Card>
        <Card className="hotbox-admin-kpi p-4"><p className="text-xs text-muted-foreground">Respostas recebidas</p><p className="mt-1 text-3xl font-extrabold">{completed.length}</p><p className="text-xs text-muted-foreground">{responseRate}% dos convites enviados</p></Card>
        <Card className="hotbox-admin-kpi p-4"><p className="text-xs text-muted-foreground">Convites enviados</p><p className="mt-1 text-3xl font-extrabold">{sent}</p><p className="text-xs text-muted-foreground">{opened} link(s) aberto(s)</p></Card>
        <Card className="hotbox-admin-kpi p-4"><p className="text-xs text-muted-foreground">Comentários</p><p className="mt-1 text-3xl font-extrabold">{completed.filter((item) => !!item.comment).length}</p><p className="text-xs text-muted-foreground">Textos opcionais recebidos</p></Card>
      </div>
