...
function ConfigPage() {
  ...
  async function save() {
    ...
    if (c.meta_pixel_script_enabled === true && !String(c.meta_pixel_script || "").trim()) {
      return toast.error("Cole o script do Meta Pixel ou desative a opção de Pixel do site.");
    }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <Card className="space-y-3 p-5" style={tabStyle("integracoes")}>
        ...
        {/* ── Meta Pixel do site: configurado 100% pelo painel ── */}
        <div className="mt-4 border-t pt-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold">Meta Pixel do site</p>
              <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">
                Não existe script de Pixel fixo no código. Cole aqui o código completo fornecido pelo Meta Events Manager; ao salvar, ele passa a ser carregado automaticamente nas páginas públicas da loja. O painel administrativo não dispara o Pixel.
              </p>
            </div>
            <Switch
              checked={(c as any).meta_pixel_script_enabled === true}
              onCheckedChange={(v) => setC({ ...c, meta_pixel_script_enabled: v } as any)}
            />
          </div>
          <div className="mt-3">
            <Label>Script completo do Meta Pixel</Label>
            <Textarea
              rows={10}
              value={(c as any).meta_pixel_script || ""}
              onChange={(e) => setC({ ...c, meta_pixel_script: e.target.value } as any)}
              placeholder={'Cole aqui o código completo que começa normalmente com <script> ...'}
              className="mt-1 font-mono text-xs"
            />
            <div className="mt-2 rounded-xl border bg-muted/30 p-3 text-[11px] leading-relaxed text-muted-foreground">
              <strong>Como funciona:</strong> salvar não altera arquivos do projeto nem exige editar código. O sistema lê esta configuração no banco e injeta o script no navegador do cliente. Para remover o Pixel, desligue a chave acima e salve.
            </div>
          </div>
        </div>
