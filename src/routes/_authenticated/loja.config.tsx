...
function ConfigPage() {
  ...
  async function save() {
    ...
    const payload = stripCardOwnedFields({
      ...
      efi_environment: c.efi_environment === "production" ? "production" : "sandbox",
      efi_max_installments: Math.min(12, Math.max(1, Number(c.efi_max_installments || 1))),
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      <Card className="space-y-4 p-5" style={tabStyle("pagamentos")}>
        ...
        <div className="hotbox-admin-page space-y-2">
      ...
      <Card className="space-y-4 p-5" style={tabStyle("pagamentos")}>
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="font-semibold">Pagar.me — checkout transparente</h2>
            <p className="mt-1 text-xs text-muted-foreground">Pix e cartão processados sem tirar o cliente da HotBox. Os dados abertos do cartão são tokenizados diretamente no Pagar.me e não passam pelo servidor da loja.</p>
          </div>
          <Switch checked={c.pagarme_enabled === true} onCheckedChange={(v) => setC({ ...c, pagarme_enabled: v })} />
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            <Label>Public Key</Label>
            <Input value={c.pagarme_public_key || ""} onChange={(e) => setC({ ...c, pagarme_public_key: e.target.value.trim() })} placeholder="pk_test_... ou pk_..." autoComplete="off" />
            <p className="mt-1 text-[11px] text-muted-foreground">Usada somente para tokenizar o cartão no navegador.</p>
          </div>
          <div>
            <Label>Secret Key</Label>
            <Input type="password" value={c.pagarme_secret_key || ""} onChange={(e) => setC({ ...c, pagarme_secret_key: e.target.value.trim() })} placeholder="sk_test_... ou sk_..." autoComplete="new-password" />
            <p className="mt-1 text-[11px] text-muted-foreground">Usada no backend para criar e confirmar pagamentos.</p>
          </div>
        </div>
        <div>
          <Label>Máximo de parcelas no cartão</Label>
          <Select value={String(c.pagarme_max_installments || 1)} onValueChange={(v) => setC({ ...c, pagarme_max_installments: Number(v) })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>{Array.from({ length: 12 }, (_, i) => i + 1).map((n) => <SelectItem key={n} value={String(n)}>{n}x</SelectItem>)}</SelectContent>
          </Select>
        </div>
        <div className="rounded-xl border bg-muted/30 p-3 text-xs leading-relaxed text-muted-foreground">
          Webhook: <code>{typeof window !== "undefined" ? `${window.location.origin}/api/public/webhooks/pagarme?token=${encodeURIComponent(String(c.pagarme_webhook_token || ""))}` : "/api/public/webhooks/pagarme"}</code>. Cadastre essa URL no painel do Pagar.me para eventos de pagamento. A HotBox confirma o pedido consultando a API do Pagar.me antes de liberar a venda.
        </div>
      </Card>
      ...
      <Card className="space-y-4 p-5" style={tabStyle("pagamentos")}>
        ...
        <div>
          ...
          <Select value={c.efi_environment === "production" ? "production" : "sandbox"} onValueChange={(v) => setC({ ...c, efi_environment: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent><SelectItem value="sandbox">🧪 Homologação</SelectItem><SelectItem value="production">🟢 Produção</SelectItem></SelectContent>
          </Select>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <div><Label>Client ID</Label><Input value={c.efi_client_id || ""} onChange={(e) => setC({ ...c, efi_client_id: e.target.value.trim() })} autoComplete="off" /></div>
          <div><Label>Client Secret</Label><Input type="password" value={c.efi_client_secret || ""} onChange={(e) => setC({ ...c, efi_client_secret: e.target.value.trim() })} autoComplete="new-password" /></div>
          <div><Label>Identificador de conta (payee_code)</Label><Input value={c.efi_payee_code || ""} onChange={(e) => setC({ ...c, efi_payee_code: e.target.value.trim() })} placeholder="Necessário para tokenizar cartão" /></div>
          <div><Label>Máximo de parcelas</Label><Select value={String(c.efi_max_installments || 1)} onValueChange={(v) => setC({ ...c, efi_max_installments: Number(v) })}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{Array.from({ length: 12 }, (_, i) => i + 1).map((n) => <SelectItem key={n} value={String(n)}>{n}x</SelectItem>)}</SelectContent></Select></div>
        </div>
        <div className="rounded-2xl border bg-zinc-50 p-4">
          <p className="font-black">Configuração Pix Efí</p>
          <p className="mt-1 text-xs text-muted-foreground">A API Pix da Efí exige uma chave Pix e certificado P12. O certificado fica apenas na configuração administrativa e nunca é enviado para o cliente.</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div><Label>Chave Pix cadastrada na Efí</Label><Input value={c.efi_pix_key || ""} onChange={(e) => setC({ ...c, efi_pix_key: e.target.value.trim() })} /></div>
            <div><Label>Senha do certificado (se houver)</Label><Input type="password" value={c.efi_pix_certificate_password || ""} onChange={(e) => setC({ ...c, efi_pix_certificate_password: e.target.value })} /></div>
            <div className="sm:col-span-2">
              <Label>Certificado P12/PFX</Label>
              <Input type="file" accept=".p12,.pfx,application/x-pkcs12" onChange={(e) => {
                const file = e.target.files?.[0]; if (!file) return;
                const reader = new FileReader(); reader.onload = () => { const raw=String(reader.result||""); setC((current:any)=>({ ...current, efi_pix_certificate_base64: raw.split(",").pop() || "" })); toast.success("Certificado Efí carregado. Salve as configurações."); }; reader.readAsDataURL(file);
              }} />
              <p className="mt-1 text-[11px] text-muted-foreground">{c.efi_pix_certificate_base64 ? "✓ Certificado carregado" : "Nenhum certificado carregado"}</p>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={async()=>{ try { const { configureEfiPixWebhook }=await import("@/lib/efi.functions"); const r:any=await configureEfiPixWebhook({data:{origin:window.location.origin}}); if(r?.ok) toast.success("Webhook Pix Efí configurado."); else toast.error(r?.error||"Não foi possível configurar o webhook."); } catch(e:any){ toast.error(e?.message||"Falha ao configurar webhook."); } }}>Configurar webhook Pix</Button>
        </div>
        <div className="rounded-xl border bg-muted/30 p-3 text-xs leading-relaxed text-muted-foreground">
          Cartão: a Efí envia um token de notificação e a HotBox consulta a API antes de liberar o pedido. Pix: a HotBox valida o txid, valor e status antes de gerar o pedido.
        </div>
      </Card>
      ...
      <Card className="space-y-3 p-5" style={tabStyle("geral")}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">Pix e cartão podem usar gateways diferentes, conforme o roteamento configurado acima. O pedido só entra na operação depois da confirmação real do pagamento.</p>
          </div>
...
function BairrosAtendidosCard({ pricingMode }: { pricingMode: "neighborhood" | "distance" }) {
  ...
  async function add() {
    ...
    const typed = String(newCeps || "").split(/[\n,;|]+/).map((item) => item.trim()).filter(Boolean);
    if (typed.length > ceps.length) return toast.error("Revise os CEPs. Cada CEP precisa ter 8 números.");
    setAdding(true);
    const fee = newFee === "" ? null : Math.max(0, Number(newFee));
    const { error } = await (supabase as any).from("bairros_atendidos").insert({ nome, ativo: true, delivery_fee: Number.isFinite(fee as number) ? fee : null, ceps });
    if (error) {
    ...
    setNewFee("");
    setNewCeps("");
    toast.success(ceps.length ? `Bairro adicionado com ${ceps.length} CEP(s)!` : "Bairro adicionado!");
    setAdding(false);
    load();
  }
  ...
  function startEdit(b: any) {
    ...
    setEditingCeps(Array.isArray(b.ceps) ? b.ceps.map((cep: string) => formatManualCep(cep)).join(", ") : "");
  }
  ...
  async function saveEdit(id: string) {
    ...
    const typed = String(editingCeps || "").split(/[\n,;|]+/).map((item) => item.trim()).filter(Boolean);
    if (typed.length > ceps.length) return toast.error("Revise os CEPs. Cada CEP precisa ter 8 números.");
    setSaving(true);
    const fee = editingFee === "" ? null : Math.max(0, Number(editingFee));
    const { error } = await (supabase as any).from("bairros_atendidos").update({ nome, delivery_fee: Number.isFinite(fee as number) ? fee : null, ceps }).eq("id", id);
    if (error) {
      toast.error(error.message.includes("duplicate") ? "Esse bairro já está cadastrado." : error.message);
      setSaving(false);
      return;
    ...
    setSaving(false);
    cancelEdit();
    toast.success("Bairro, taxa e CEPs atualizados!");
    load();
  }
  ...
  return (
    <Card className="space-y-4 p-5">
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Esta é a lista oficial de bairros com entrega própria. Você também pode liberar <strong>CEPs manualmente</strong> dentro de cada bairro. O CEP manual funciona como uma liberação extra e não restringe os demais endereços do bairro. {pricingMode === "neighborhood" ? (<>No modo <strong>Por bairro</strong>, a taxa cadastrada em cada linha é usada no cardápio.</>) : (<>No modo <strong>Por quilometragem</strong>, os bairros/CEPs definem a área permitida e o valor é calculado pelas faixas de km.</>)}
        </p>
      </div>

      {/* form para adicionar */}
      <div className="rounded-xl border bg-muted/20 p-3">
        <div className="grid gap-3 lg:grid-cols-[1fr_150px_1.4fr_auto] lg:items-end">
          <div><Label className="text-xs">Bairro atendido</Label><Input placeholder="Ex.: Vila São Luís" value={newNome} onChange={(e) => setNewNome(e.target.value)} className="mt-1 rounded-xl text-sm" /></div>
          <div><Label className="text-xs">Taxa</Label><div className="relative mt-1"><span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-muted-foreground">R$</span><Input type="number" min="0" step="0.01" placeholder="Taxa" value={newFee} onChange={(e) => setNewFee(e.target.value)} className="rounded-xl pl-9 text-sm" /></div></div>
          <div><Label className="text-xs">CEPs liberados manualmente <span className="font-normal text-muted-foreground">(opcional)</span></Label><Input placeholder="Ex.: 25050-123, 25051-240" value={newCeps} onChange={(e) => setNewCeps(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} className="mt-1 rounded-xl text-sm" /><p className="mt-1 text-[11px] text-muted-foreground">Separe vários CEPs por vírgula.</p></div>
          <Button size="sm" onClick={add} disabled={adding || !newNome.trim()} className="lg:mb-[18px]"><Plus className="size-4" /> Adicionar</Button>
        </div>
      </div>
            ...
            >
              ...
              {editingId === b.id ? (
                <>
                  ...
                  <Input value={editingCeps} onChange={(e) => setEditingCeps(e.target.value)} placeholder="CEPs: 25050-123, 25051-240" className="h-8 min-w-[220px] flex-[1.2] rounded-lg text-sm" />
                  <Button
                    size="icon"
                    variant="ghost"
                ...
                <>
                  <div className="min-w-0 flex-1"><p className="text-sm font-semibold leading-snug">{b.nome}</p><p className="mt-0.5 text-[11px] text-muted-foreground">Taxa: <strong className="text-foreground">{b.delivery_fee == null ? "padrão" : `R$ ${Number(b.delivery_fee).toFixed(2).replace(".", ",")}`}</strong>{pricingMode === "distance" ? " • ignorada enquanto km estiver ativo" : ""}</p>{Array.isArray(b.ceps) && b.ceps.length > 0 ? <div className="mt-1.5 flex flex-wrap gap-1">{b.ceps.map((cep: string) => <span key={cep} className="rounded-full border bg-background px-2 py-0.5 text-[10px] font-semibold">CEP {formatManualCep(cep)}</span>)}</div> : <p className="mt-1 text-[10px] text-muted-foreground">Sem CEP manual — acesso pelo nome do bairro.</p>}</div>
