...
function AdminLayout() {
  ...
  useEffect(() => {
    ...
    async function checkLiveVisitors() {
      ...
      const deliveryRows = (data || []).filter((row: any) => {
        const raw = String(row?.current_page_path || "/").split("?")[0].replace(/\/+$/, "") || "/";
      ...
      const pageParts = [...pageCounts.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, 2)
        .map(([label, count]) => `${count} ${label}`);
      if (deliveryCount > 0) pageParts.push(`${deliveryCount} verificando entrega`);
      ...
      if (previousCount === null) {
        ...
        if (currentCount > 0) {
          ...
          showLiveAdminBubble({
            kind: "visitors",
            title: deliveryCount === 1 ? "Cliente verificando entrega" : "Clientes verificando entrega",
            message: `${deliveryCount} ${deliveryCount === 1 ? "pessoa chegou" : "pessoas chegaram"} à etapa de CEP/bairro depois de montar a sacola.`,
          });
        }
        return;
      }
