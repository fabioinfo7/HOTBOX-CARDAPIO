...
export const Route = createFileRoute("/_authenticated/loja/pedido/$id")({
  ...
  validateSearch: (search: Record<string, unknown>) => {
    const out: { print?: string; mode?: "edit" | "add" } = {};
    if (typeof search.print === "string") out.print = search.print;
    if (search.mode === "edit" || search.mode === "add") out.mode = search.mode;
    return out;
  },
});
...
function OrderDetail() {
  ...
  const { print: autoPrint, mode: openMode } = Route.useSearch();
  ...
  useEffect(() => {
    ...
    if (openMode === "add") {
      window.setTimeout(() => {
        document.getElementById("order-items-editor")?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }, 150);
  ...
  async function saveEditDraft() {
    ...
    try {
      ...
      const requestedTotal = Number(editDraft.total);
      if (Number.isFinite(requestedTotal) && requestedTotal >= 0) {
        const { error: totalError } = await supabase
          .from("orders")
          .update({ total: requestedTotal })
          .eq("id", id);
        if (totalError) throw totalError;
        setOrder((prev: any) => (prev ? { ...prev, total: requestedTotal } : prev));
      }
  ...
  async function addManualItemToOrder() {
    ...
    const unitPrice = Math.max(0, Number(String(manualItemPrice).replace(",", ".")) || 0);
    const notes = manualItemNotes.trim();

    if (!name) return toast.error("Digite o nome do item manual.");
    if (!String(manualItemPrice).trim()) return toast.error("Informe o preço do item manual.");
    ...
    try {
      const { error } = await supabase.from("order_items").insert({
        ...
        notes: notes || "Item adicionado manualmente pela loja",
      });
      if (error) throw error;
      ...
      setManualItemNotes("");
      await syncOrderTotals();
      toast.success("Item manual adicionado e total recalculado");
    } catch (err: any) {
      toast.error(err?.message || "Não foi possível adicionar o item manual");
    } finally {
    ...
    <div className="space-y-4">
      ...
      <div className="no-print grid gap-4 lg:grid-cols-3">
        <Card className="overflow-hidden rounded-2xl p-0 shadow-sm lg:col-span-2">
          ...
          <div id="order-items-editor" className="scroll-mt-24 space-y-2.5 px-6 py-5">
            ...
            <div className="rounded-xl border border-dashed bg-amber-50/40 p-3">
              <div className="mb-2">
                <p className="text-sm font-bold">Adicionar qualquer item manualmente</p>
                <p className="text-[11px] text-muted-foreground">
                  Use para adicionais, bebidas, cortesias, cobranças extras ou qualquer item que não esteja no cardápio.
                </p>
              </div>

              <div className="grid gap-2 md:grid-cols-[1.6fr_80px_120px_1.4fr_auto] md:items-end">
                <div>
                  <p className="mb-1 text-[10px] font-bold uppercase text-muted-foreground">Nome</p>
                  <Input
                ...
                <div>
                  <p className="mb-1 text-[10px] font-bold uppercase text-muted-foreground">Qtd.</p>
                  <Input
                    type="number"
                    min={1}
                ...
                <div>
                  <p className="mb-1 text-[10px] font-bold uppercase text-muted-foreground">Preço unit.</p>
                  <Input
                    inputMode="decimal"
                ...
                <div>
                  <p className="mb-1 text-[10px] font-bold uppercase text-muted-foreground">Observação / adicional</p>
                  <Input
                    value={manualItemNotes}
                    onChange={(e) => setManualItemNotes(e.target.value)}
                    placeholder="Ex.: adicional solicitado no WhatsApp"
                    className="h-9"
                  />
                </div>
                <Button
                  ...
                  disabled={addingManualItem || !manualItemName.trim()}
                >
                  <PlusCircle className="size-4" />
                  {addingManualItem ? "Adicionando..." : "Adicionar manual"}
                </Button>
              </div>
            </div>
            {items.map((i) => (
              ...
              >
                ...
                <div className="mt-2">
                  <p className="mb-1 text-[10px] font-bold uppercase text-muted-foreground">Observação / adicionais</p>
                  <Input
                    className="h-8 bg-background text-xs"
                    defaultValue={i.notes ?? ""}
                    placeholder="Ex.: Bacon extra, sem cebola..."
                    onBlur={(e) => editItem(i.id, { notes: e.target.value.trim() || null })}
                  />
                </div>
              </div>
            ))}
        ...
        <div className="space-y-4">
          ...
          <Card className="rounded-2xl p-5 shadow-sm">
            ...
            {order.payment_timing === "delivery" && order.payment_status !== "paid" && (
              <div className="mb-4 rounded-2xl border-2 border-amber-500 bg-amber-50 p-4 shadow-sm">
                <p className="text-xs font-black uppercase tracking-[0.16em] text-amber-950">⚠ PAGAMENTO NA ENTREGA</p>
                <p className="mt-1 text-base font-black text-amber-900">
                  Receber {order.payment_method === "pix" ? "por Pix" : "no cartão"} • {brl(order.total)}
                </p>
                <p className="mt-1 text-xs font-semibold text-amber-800">O pedido foi criado sem cobrança online e deve permanecer pendente até o recebimento.</p>
              </div>
            )}
