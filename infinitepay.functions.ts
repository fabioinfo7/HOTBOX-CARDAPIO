...
export const confirmInfinitePayReturn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (alreadyCreatedOrderId) return { ok: true, order_id: alreadyCreatedOrderId, checkout_id: String(checkout.id), total: Number(checkout.total || 0), payment_method: paymentKind };
    ...
    return { ok: true, order_id: finalized.order_id, checkout_id: String(checkout.id), total: Number(checkout.total || 0), payment_method: paymentKind };
  });
