...
function ObrigadoPage() {
  ...
  useEffect(() => {
    ...
    async function confirmPayOnDelivery() {
      ...
      if (directOrderId) setOrderId(directOrderId);
  ...
  useEffect(() => {
    if ((state !== "paid" && state !== "delivery") || !orderId) return;
    const key = `hb_analytics_purchase_${orderId}`;
    if (sessionStorage.getItem(key) === "1") return;
    sessionStorage.setItem(key, "1");
    const params = new URLSearchParams(window.location.search);
    trackAnalytics("purchase_completed", { category: "conversion", checkoutId: params.get("checkout_id") || params.get("order_nsu"), orderId, metadata: { surface: "thankyou", provider: params.get("provider") || "infinitepay", payment_state: state } });
  }, [state, orderId]);
